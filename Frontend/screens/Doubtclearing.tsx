...
const DoubtClearingScreen = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setLoading(false);
      // Optional: Dismiss the keyboard after sending to ensure a clean UI state
      Keyboard.dismiss();
    }
  };
...
const markdownStyles = StyleSheet.create({
  body: { fontSize: 16, color: '#4A4135' },
  code_block: { backgroundColor: '#E8DBC6', color: '#4A4135', padding: 10, borderRadius: 4, fontFamily: Platform.OS === 'ios' ? 'Courier New' : 'monospace' },
  heading1: { fontSize: 24, fontWeight: 'bold', color: '#4A4135' },
...
const styles = StyleSheet.create({
  ...
  tutorMessage: { backgroundColor: '#FFFFFF', alignSelf: 'flex-start', borderBottomLeftRadius: 5 },
  userMessageText: { color: '#fff', fontSize: 16 },
  inputContainer: { flexDirection: 'row', alignItems: 'center', padding: 10, borderTopWidth: 1, borderTopColor: '#A19A8F', backgroundColor: '#FCF0DB' },
  input: { 
    flex: 1, 
  ...
  typingText: { marginLeft: 10, color: '#A19A8F', fontStyle: 'italic' },
  emptyContainer: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  emptyText: { marginTop: 10, fontSize: 20, fontWeight: 'bold', color: '#A19A8F' },
});
