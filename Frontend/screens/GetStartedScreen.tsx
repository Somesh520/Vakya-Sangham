  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      <MotiView from={{opacity: 0}} animate={{opacity: 1}}>
        ...
        <ProgressBar progress={0.6} color={styles.progressFill.backgroundColor} style={styles.progressBar} />
      </MotiView>
      ...
      <MotiView from={{opacity: 0, translateX: -20}} animate={{opacity: 1, translateX: 0}} transition={{delay: 100}}>
        ...
        <RadioButton.Group onValueChange={newValue => setGoal(newValue)} value={goal}>
          {goals.map((item) => (
            <RadioButton.Item 
              key={item}
              label={item}
              ...
              style={goal === item ? styles.selectedOption : styles.option}
            />
          ))}
        </RadioButton.Group>
      </MotiView>
      ...
      <MotiView from={{opacity: 0, translateX: -20}} animate={{opacity: 1, translateX: 0}} transition={{delay: 200}}>
        ...
        <RadioButton.Group onValueChange={newValue => setContentPreference(newValue)} value={contentPreference}>
          {courseTypes.map((type) => (
             <RadioButton.Item 
              ...
              labelStyle={styles.optionText}
              color={styles.radioSelected.backgroundColor}
              style={contentPreference === type ? styles.selectedOption : styles.option}
            />
          ))}
        </RadioButton.Group>
      </MotiView>
